...
$sql_enderecoImovel = 'UPDATE trancoso_imoveis SET imovelRua = :imovelRua, imovelNumero = :imovelNumero, 
...
$imovelId = $_POST['imovelId'];
$sql_limitaUpload = 'SELECT * FROM trancoso_midias WHERE imovelId = :imovelId';
try{
	...
	if($count_limitaUpload >= '5'){
	  echo '<h1>Você já enviou 5 Imagens de 5 Imagens!';	
	}else{
...
for($i=0;$i<$contarImg;$i++){
	...
	if(!empty($imagemNome) && in_array($imagemTipo, $imgPermitido)){
	   ...
	   $sql_cadastraImagem  = 'INSERT INTO trancoso_midias (imovelId, imovelImg) ';
...
sleep(1);
$meta = '<meta http-equiv="refresh" content="0, URL=painel.php?exe=imoveis-cadastro/cadastro-fim" />';
echo $meta;	
}?>
      ...
      <h2>Você pode enviar até 5 imagens!</h2>
...
$sql_deletaImg = 'DELETE FROM  trancoso_midias WHERE fotoId = :fotoId';
try{
...
<?php
$sql_pegaImagem = 'SELECT * FROM trancoso_midias WHERE imovelId = :imovelId';
try{
