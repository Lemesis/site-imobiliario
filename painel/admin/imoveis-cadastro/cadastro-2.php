...
$datafinal = date('Y-m-d H:m:s',strtotime('+1 year'));
...
for($i=0;$i<$contarImg;$i++){
	...
	if(!empty($imagemNome) && in_array($imagemTipo, $imgPermitido)){
	   ...
	   $sql_cadastraImvel  = 'INSERT INTO trancoso_imoveis (imovelVisitas, imovelTitulo,imovelThumb, imovelTipo, imovelValor, imovelNegocio,
	                          imovelDescricao, imovelComodos, imovelSuites, imovelBanheiros, imovelSalas, imovelCadastro, imovelUpdate, imovelTermino, imovelStatus) ';
	   $sql_cadastraImvel .= 'VALUES (:imovelVisitas, :imovelTitulo, :imovelThumb, :imovelTipo, :imovelValor, :imovelNegocio,
	                          :imovelDescricao, :imovelComodos, :imovelSuites, :imovelBanheiros, :imovelSalas, :imovelCadastro, :imovelUpdate, :imovelTermino, :imovelStatus)';
...
?>
      ...
      <form name="cadastraImovelCliente-2" id="cadastraImovelCliente-2" action="painel.php?exe=imoveis-cadastro/cadastro-3" method="post" enctype="multipart/form-data">
